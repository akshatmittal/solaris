...
const moveFocusWithin = (element: HTMLElement, position: "start" | "end") => {
  ...
  const focusTarget = focusableElements.item(position === "end" ? focusableElements.length - 1 : 0);
  focusTarget?.focus();
};

export function FocusTrap(props: JSX.IntrinsicElements["div"]) {
  const contentRef = useRef<HTMLDivElement>(null);
  const sentinelStyle = { border: 0, height: 0, opacity: 0, padding: 0, position: "fixed", width: 0 } as const;
