...
import { ENHANCED_PROVIDER_ENABLED, enhancedProviderHttp } from "./enhancedProvider";
...
describe("createHttpClient", () => {
  ...
  it("should throw an error if endpoint doesn't exist", async () => {
    await expect(() => enhancedProviderHttp.get("/unknown")).rejects.toThrow("Not Found");
  });

  it("should resolve ENS name for a valid address", async () => {
    const address = "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045";
    const { data } = await enhancedProviderHttp.get<{ data: string | null }>("/v1/resolve-ens", {
      params: { address },
    });
