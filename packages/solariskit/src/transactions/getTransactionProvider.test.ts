...
function createReceipt() {
  return { status: "success" } as Awaited<ReturnType<typeof waitForTransactionReceipt>>;
}

function createProvider(uid: string) {
  const receipts: ReturnType<typeof createDeferred<ReturnType<typeof createReceipt>>>[] = [];
...
describe("getTransactionProvider", () => {
  it("uses a Solaris transaction watcher uid", () => {
