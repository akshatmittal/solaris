...
On storage recovery, replay unsaved local transaction mutations against the latest stored data to preserve other tabs' unrelated transactions. Local mutations take precedence for the same hash, and local clears apply to their account and chain. Keep storage reads guarded without runtime validation of the stored transaction schema.
